	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try (Connection con = new Conector().getMySQLConnection()) {
			...
			try (PreparedStatement prepStmt = con.prepareStatement(
				...
				response.sendRedirect("/clinicaweb/jsp/especialistas.jsp");

			} catch (SQLException e) {
				e.printStackTrace();

				request.setAttribute("Mensaje", "El usuario no ha podido crearse");
				RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/clinicaweb/jsp/especialistas.jsp");
